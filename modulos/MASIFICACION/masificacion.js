(function () {
  ...
  let datosDashboardNagasco={titulo:'Dashboard comparativo Nagasco',periodo:'Cargando datos…',proyectos:[]};
  function renderDashboardMasificacion(){
    const proyectos=(datosDashboardNagasco.proyectos||[]).map(item=>({proyecto:item.proyecto,libros:Number(item.libros)||0,parcial:Number(item.montoParcial)||0,finalRegistrado:Number(item.montoFinalRegistrado)||0,finalEsperado:(Number(item.montoParcial)||0)*.25,total:(Number(item.montoParcial)||0)*1.25,librosFinal:Number(item.librosFinal)||0}));
    ...
    $('tituloDashboardMasificacion').textContent=datosDashboardNagasco.titulo||'Dashboard comparativo Nagasco';
  ...
  function actualizarConfiguracionActoresComponentes(){
    ...
    document.querySelectorAll('.gestion-contractual-componente').forEach(tarjeta=>{
      ...
      tarjeta.hidden=!independientes&&!esPsr;
      if(!titulo)return;
  ...
  function iniciar() {
    ...
    document.querySelectorAll('input[name="actoresCompartidosComponentes"]').forEach(opcion=>opcion.addEventListener('change',actualizarConfiguracionActoresComponentes));
