(function () {
  ...
  const mostrarVistaCronogramaProyecto = () => {
    ...
    mostrarPasoAsistenteProyecto(4);
    window.scrollTo({top:0,behavior:'smooth'});
  };
  ...
  let mapaUbicacionProyecto=null,mapaResumenProyecto=null,capaPoligonoUbicacion=null,capaTerritorialUbicacion=null,puntosPoligonoUbicacion=[],origenMapaCrearProyecto=false,nivelMapaProyecto='pais',departamentoMapaProyecto=null,provinciaMapaProyecto=null,distritoMapaProyecto=null,terrenoActivoProyecto='A';
  ...
  function organizarBloquesGestionContractual(){document.querySelectorAll('.campos-gestion-contractual').forEach(contenedor=>{if(contenedor.dataset.organizado)return;const campos=[...contenedor.querySelectorAll(':scope>label')],crearGrupo=(titulo,marca,camposGrupo)=>{const grupo=document.createElement('section');grupo.className='grupo-actor-contractual';grupo.innerHTML=`<header><strong>${titulo}</strong><small>${marca}</small></header>`;camposGrupo.forEach(campo=>grupo.append(campo));return grupo;},fila=document.createElement('div'),contrato=document.createElement('div');fila.className='fila-actores-contractuales';contrato.className='fila-contrato-componentes';fila.append(crearGrupo('Contratista','Obligatorio',campos.slice(0,4)),crearGrupo('Interventor','Obligatorio',campos.slice(4,8)));const petroperu=document.createElement('section');petroperu.className='grupo-actor-contractual';petroperu.innerHTML='<header><strong>Supervisor PETROPERÚ</strong><small>Según contrato</small></header><label class="opcion-actor-contractual"><input type="checkbox"> Activar participación de PETROPERÚ</label><label>Responsable / área<input type="text" placeholder="Supervisor / unidad"></label><label>Correo<input type="email" placeholder="correo@petroperu.com.pe"></label><label>Tipo de participación<select><option>Conformidad técnica supervisada</option><option>Validación contractual</option><option>Visita de campo</option></select></label>';const osinergmin=document.createElement('section');osinergmin.className='grupo-actor-contractual';osinergmin.innerHTML='<header><strong>OSINERGMIN</strong><small>Según etapa</small></header><label class="opcion-actor-contractual"><input type="checkbox"> Requiere intervención regulatoria</label><label>Tipo<select><option>Inspección de seguridad / cumplimiento</option><option>Informe de supervisión</option><option>Fiscalización</option></select></label><label class="campo-largo">Qué verifica<textarea rows="3" placeholder="Alcance de la supervisión o verificación"></textarea></label>';fila.append(petroperu,osinergmin);contrato.innerHTML='<strong>Información del contrato</strong>';campos.slice(8).forEach(campo=>contrato.append(campo));contenedor.replaceChildren(fila,contrato);contenedor.dataset.organizado='true';});}
  ...
  function cerrarFormulariosComponenteFise(){if($('modalFormulariosComponenteFise').open)$('modalFormulariosComponenteFise').close();mostrarPasoAsistenteProyecto(3);}
  function mostrarPasoAsistenteProyecto(paso){pasoAsistenteProyecto=paso;if(vistaCrearProyecto.hidden)mostrarVistaCrearProyecto();const esFormularios=paso===3;$('pasoDatosProyecto').hidden=paso!==1;$('pasoComponentesProyecto').hidden=paso!==2&&!esFormularios;$('pasoComponentesProyecto').classList.toggle('modo-formularios',esFormularios);$('pasoBeneficiariosProyecto').hidden=true;$('pasoResumenProyecto').hidden=paso!==5;vistaCronogramaProyecto.hidden=paso!==4;$('anteriorAsistenteProyecto').hidden=true;$('cancelarCrearProyecto').hidden=true;$('guardarBorradorProyecto').hidden=true;$('continuarAsistenteProyecto').hidden=true;$("guardarCrearProyecto").hidden=paso!==5;vistaCrearProyecto.querySelector('.acciones-crear-proyecto').hidden=paso!==5;document.querySelector('.boton-subir-beneficiarios').hidden=!proyectoEdicionSeleccionado;actualizarIndicadorAsistente(paso);$('etiquetaCrearProyecto').textContent=`PASO ${paso} DE 5`;$('tituloCrearProyecto').textContent=paso===1?'Registro de ficha de proyecto':paso===2?'Gestión contractual':paso===3?'Formularios FISE':paso===4?'Cronograma':'Resumen final';$('descripcionCrearProyecto').textContent=paso===1?'Delimite el área GIS y complete las fechas, equipo y alcance del proyecto.':paso===2?'Registre contratista, interventora, contrato y documentos de cada componente.':paso===3?'Complete los formularios FISE de Redes, PSR-GNL y Tuberías de conexión.':paso===4?'Organice las actividades y fechas del proyecto.':'Revise todos los datos antes de crear el proyecto.';if(paso===3)renderComponentesProyecto();if(paso===5)renderResumenFinalProyecto();}
  ...
  function renderPoligonoUbicacion(){if(!capaPoligonoUbicacion)return;capaPoligonoUbicacion.clearLayers();const paletas={A:{linea:'#36a8d2',relleno:'#58c5e8'},B:{linea:'#e68b3f',relleno:'#f2b65a'}};['A','B'].forEach(codigo=>{const geometria=leerGeometriaTerreno(codigo);if(codigo===terrenoActivoProyecto&&puntosPoligonoUbicacion.length)return;if(!geometria?.coordenadas?.length)return;const estilo=paletas[codigo];L.polygon(geometria.coordenadas,{color:estilo.linea,fillColor:estilo.relleno,fillOpacity:.31,weight:3}).bindTooltip(`Terreno ${codigo}`,{permanent:true,direction:'center',className:`etiqueta-terreno etiqueta-terreno-${codigo.toLowerCase()}`}).addTo(capaPoligonoUbicacion);});const estiloActivo=paletas[terrenoActivoProyecto];puntosPoligonoUbicacion.forEach((p,i)=>L.circleMarker(p,{radius:5,color:'#fff',weight:2,fillColor:estiloActivo.relleno,fillOpacity:1}).bindTooltip(`Vértice ${i+1}`).addTo(capaPoligonoUbicacion));if(puntosPoligonoUbicacion.length>1)L.polyline(puntosPoligonoUbicacion,{color:estiloActivo.linea,weight:3,dashArray:puntosPoligonoUbicacion.length<3?'6 5':null}).addTo(capaPoligonoUbicacion);if(puntosPoligonoUbicacion.length>=3)L.polygon(puntosPoligonoUbicacion,{color:estiloActivo.linea,fillColor:estiloActivo.relleno,fillOpacity:.32,weight:3}).bindTooltip(`Terreno ${terrenoActivoProyecto}`,{permanent:true,direction:'center',className:`etiqueta-terreno etiqueta-terreno-${terrenoActivoProyecto.toLowerCase()}`}).addTo(capaPoligonoUbicacion);$('estadoZonaProyecto').textContent=puntosPoligonoUbicacion.length>=3?`${puntosPoligonoUbicacion.length} vértices registrados. El polígono está listo para guardar.`:'';}
  ...
  function guardarMapaUbicacionProyecto(){
    if(puntosPoligonoUbicacion.length<3)return false;
    ...
    const centro=puntosPoligonoUbicacion.reduce((a,p)=>({lat:a.lat+p.lat/puntosPoligonoUbicacion.length,lng:a.lng+p.lng/puntosPoligonoUbicacion.length}),{lat:0,lng:0}),nombre=$('nombreZonaProyecto').value.trim()||`Terreno ${terrenoActivoProyecto}`,coords=puntosPoligonoUbicacion.map(p=>[Number(p.lat.toFixed(6)),Number(p.lng.toFixed(6))]);let suma=0;for(let i=0;i<coords.length;i++){const a=coords[i],b=coords[(i+1)%coords.length];suma+=a[1]*b[0]-b[1]*a[0];}
    const areaM2=Math.max(1,Math.round(Math.abs(suma)*.5*111320*111320*Math.cos(centro.lat*Math.PI/180))),beneficiariosEstimados=Math.max(1,Math.round(areaM2/120)),geometria={tipo:'Polígono',nombre,categoria:$('tipoZonaProyecto').value,coordenadas:coords,areaM2,beneficiariosEstimados};
  ...
  function iniciar() {
    ...
    document.querySelector('.navegacion-etapas-proyecto').addEventListener('click',evento=>{
      ...
      if(paso===4)renderCronogramaProyecto();
    ...
    $('continuarAsistenteProyecto').addEventListener('click',()=>{if(pasoAsistenteProyecto===1){mostrarPasoAsistenteProyecto(2);return;}if(pasoAsistenteProyecto===2){mostrarPasoAsistenteProyecto(3);return;}if(pasoAsistenteProyecto===3){renderCronogramaProyecto();mostrarPasoAsistenteProyecto(4);}});
    $('anteriorAsistenteProyecto').addEventListener('click',()=>{if(pasoAsistenteProyecto===2)mostrarPasoAsistenteProyecto(1);else if(pasoAsistenteProyecto===3)mostrarPasoAsistenteProyecto(2);else if(pasoAsistenteProyecto===5){renderCronogramaProyecto();mostrarPasoAsistenteProyecto(4);}});
    ...
    $('continuarPlanificacionAsistente').addEventListener('click',()=>{leerPlanificacionFormulario();renderCronogramaProyecto();mostrarPasoAsistenteProyecto(4);});
    ...
    $('continuarCronogramaAsistente').addEventListener('click',()=>{leerCronogramaProyecto();mostrarPasoAsistenteProyecto(5);});
    ...
    $('subirConvenioProyecto').addEventListener('click',()=>$('proyectoArchivoConvenio').click());$('proyectoArchivoConvenio').addEventListener('change',evento=>{$('estadoArchivoConvenio').textContent=evento.target.files[0]?`${evento.target.files[0].name} adjunto.`:'Sin convenio adjunto.';});
    ...
    $('abrirMapaUbicacionProyecto').addEventListener('click',abrirTerrenosInfluencia);$('seleccionarTerrenoA').addEventListener('click',()=>seleccionarTerrenoEnMapa('A'));$('seleccionarTerrenoB').addEventListener('click',()=>seleccionarTerrenoEnMapa('B'));$('editarTerrenoMapa').addEventListener('click',()=>seleccionarTerrenoEnMapa(terrenoActivoProyecto,true));
