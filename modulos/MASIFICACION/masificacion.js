(function () {
  ...
  let mapaUbicacionProyecto=null,mapaResumenProyecto=null,capaPoligonoUbicacion=null,capaTerritorialUbicacion=null,puntosPoligonoUbicacion=[],circuloDibujoUbicacion=null,modoDibujoAreaProyecto='poligono',origenMapaCrearProyecto=false,nivelMapaProyecto='pais',departamentoMapaProyecto=null,provinciaMapaProyecto=null,distritoMapaProyecto=null,terrenoActivoProyecto='A';
  ...
  function mostrarPasoAsistenteProyecto(paso){pasoAsistenteProyecto=paso;vistaCrearProyecto.dataset.pasoActivo=paso;vistaCronogramaProyecto.dataset.pasoActivo=paso;if(vistaCrearProyecto.hidden)mostrarVistaCrearProyecto();const esParametros=paso===3,esFormularios=paso===4;$('pasoDatosProyecto').hidden=paso!==1;$('pasoComponentesProyecto').hidden=paso!==2&&!esParametros&&!esFormularios;$('pasoComponentesProyecto').classList.toggle('modo-formularios',esFormularios);document.querySelector('[data-panel-gestion-contractual="actores"]')?.toggleAttribute('hidden',paso!==2);document.querySelector('[data-panel-gestion-contractual="parametros"]')?.toggleAttribute('hidden',!esParametros);document.querySelector('.separador-formularios-componentes')?.toggleAttribute('hidden',!esFormularios);$('listaComponentesProyecto').hidden=!esFormularios;$('pasoBeneficiariosProyecto').hidden=true;$('pasoResumenProyecto').hidden=paso!==6;vistaCronogramaProyecto.hidden=paso!==5;$('anteriorAsistenteProyecto').hidden=true;$('cancelarCrearProyecto').hidden=true;$('guardarBorradorProyecto').hidden=true;$('continuarAsistenteProyecto').hidden=true;$("guardarCrearProyecto").hidden=paso!==6;vistaCrearProyecto.querySelector('.acciones-crear-proyecto').hidden=paso!==6;document.querySelector('.boton-subir-beneficiarios').hidden=!proyectoEdicionSeleccionado;actualizarIndicadorAsistente(paso);$('etiquetaCrearProyecto').textContent=`PASO ${paso} DE 6`;$('tituloCrearProyecto').textContent=paso===1?'Registro de ficha de proyecto':paso===2?'Gestión contractual':paso===3?'Parámetros contractuales':paso===4?'Formularios FISE':paso===5?'Cronograma':'Resumen final';$('descripcionCrearProyecto').textContent=paso===1?'Delimite el área GIS y complete las fechas, equipo y alcance del proyecto.':paso===2?'Registre contratista, interventora, contrato y documentos de cada componente.':paso===3?'Registre hitos de PSR y baremos aplicables para Redes y TCs.':paso===4?'Complete los formularios FISE de Redes, PSR-GNL y Tuberías de conexión.':paso===5?'Organice las actividades y fechas del proyecto.':'Revise todos los datos antes de crear el proyecto.';if(paso===4)renderComponentesProyecto();if(paso===6)renderResumenFinalProyecto();}
  ...
  function iniciar() {
    ...
    document.querySelector('.navegacion-etapas-proyecto').addEventListener('click',evento=>{
      ...
      if(paso===5)renderCronogramaProyecto();
    ...
    $('continuarAsistenteProyecto').addEventListener('click',()=>{if(pasoAsistenteProyecto===1){mostrarPasoAsistenteProyecto(2);return;}if(pasoAsistenteProyecto===2){mostrarPasoAsistenteProyecto(3);return;}if(pasoAsistenteProyecto===3){mostrarPasoAsistenteProyecto(4);return;}if(pasoAsistenteProyecto===4){renderCronogramaProyecto();mostrarPasoAsistenteProyecto(5);}});
    $('anteriorAsistenteProyecto').addEventListener('click',()=>{if(pasoAsistenteProyecto===2)mostrarPasoAsistenteProyecto(1);else if(pasoAsistenteProyecto===3)mostrarPasoAsistenteProyecto(2);else if(pasoAsistenteProyecto===4)mostrarPasoAsistenteProyecto(3);else if(pasoAsistenteProyecto===6){renderCronogramaProyecto();mostrarPasoAsistenteProyecto(5);}});
    ...
    $('continuarPlanificacionAsistente').addEventListener('click',()=>{leerPlanificacionFormulario();renderCronogramaProyecto();mostrarPasoAsistenteProyecto(5);});
    ...
    $('continuarCronogramaAsistente').addEventListener('click',()=>{leerCronogramaProyecto();mostrarPasoAsistenteProyecto(6);});
    ...
    const renderDocumentosConvenio=()=>{const archivos=[...$('proyectoArchivoConvenio').files];$('listaDocumentosConvenio').innerHTML=archivos.map((archivo,indice)=>`<tr><td>${escapePlanificacion(archivo.name)}</td><td><button type="button" data-ver-pdf-convenio="${indice}">👁 Ver</button></td></tr>`).join('');};
    $('subirConvenioProyecto').addEventListener('click',()=>{cargandoConvenio=true;grupoArchivosActorActivo=null;$('tituloArchivosActor').textContent='Adjuntar Convenio FISE – GORE';$('ayudaArchivosActor').textContent='Cargue uno o varios convenios firmados en formato PDF.';$('estadoArchivosActor').textContent='Arrastre los PDF o selecciónelos desde su equipo.';$('modalArchivosActor').showModal();});$('proyectoArchivoConvenio').addEventListener('change',evento=>{const archivos=[...evento.target.files];urlsVistaPdfConvenio.forEach(url=>URL.revokeObjectURL(url));urlsVistaPdfConvenio=archivos.map(archivo=>URL.createObjectURL(archivo));$('verConvenioProyecto').disabled=!archivos.length;$('estadoArchivoConvenio').textContent=archivos.length?`${archivos.length} PDF adjunto${archivos.length===1?'':'s'}.`:'Sin convenio adjunto.';});
    $('verConvenioProyecto').addEventListener('click',()=>{if(!$('proyectoArchivoConvenio').files.length)return;renderDocumentosConvenio();$('nombreVistaPdfConvenio').textContent='Seleccione un documento para visualizarlo.';$('estadoVistaPdfConvenio').hidden=true;$('visorPdfConvenio').hidden=true;$('modalVistaPdfConvenio').showModal();});$('listaDocumentosConvenio').addEventListener('click',evento=>{const boton=evento.target.closest('[data-ver-pdf-convenio]');if(!boton)return;const indice=Number(boton.dataset.verPdfConvenio),archivo=$('proyectoArchivoConvenio').files[indice];if(!archivo)return;$('nombreVistaPdfConvenio').textContent=archivo.name;$('visorPdfConvenio').src=urlsVistaPdfConvenio[indice];$('visorPdfConvenio').hidden=false;});$('cerrarVistaPdfConvenio').addEventListener('click',cerrarVistaPdfConvenio);$('cancelarVistaPdfConvenio').addEventListener('click',cerrarVistaPdfConvenio);
