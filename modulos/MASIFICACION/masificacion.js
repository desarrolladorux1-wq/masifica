(function () {
  ...
  let mapaUbicacionProyecto=null,capaPoligonoUbicacion=null,capaTerritorialUbicacion=null,puntosPoligonoUbicacion=[],origenMapaCrearProyecto=false,nivelMapaProyecto='pais',departamentoMapaProyecto=null,provinciaMapaProyecto=null,distritoMapaProyecto=null,terrenoActivoProyecto='A';
  ...
  function guardarMapaUbicacionProyecto(){
    ...
    if(terrenoActivoProyecto==='A'){
      $('proyectoAreaInfluencia').value=`${nombre} · ${areaM2.toLocaleString('es-PE')} m²`;$('proyectoLocalizacion').value=`${centro.lat.toFixed(6)}, ${centro.lng.toFixed(6)}`;$('proyectoGeometria').value=JSON.stringify(geometria);$('proyectoDepartamento').value=departamentoSeleccionado;poblarUbicacionesProyecto();$('proyectoProvincia').value=$('mapaProvinciaProyecto').value;poblarUbicacionesProyecto();establecerValorUbicacion('proyectoDistrito',$('mapaDistritoProyecto').value);$('resumenAreaInfluenciaProyecto').textContent=`Terreno A · ${areaM2.toLocaleString('es-PE')} m² · ${beneficiariosEstimados.toLocaleString('es-PE')} beneficiarios estimados`;
    }else{$('proyectoGeometriaTerrenoB').value=JSON.stringify(geometria);$('proyectoBeneficiariosTerrenoB').value=beneficiariosEstimados;$('estadoArchivoAreaIntervencion').textContent=`Terreno B dibujado · ${areaM2.toLocaleString('es-PE')} m² · ${beneficiariosEstimados.toLocaleString('es-PE')} beneficiarios estimados.`;}
    const beneficiariosA=terrenoActivoProyecto==='A'?beneficiariosEstimados:Number(JSON.parse($('proyectoGeometria').value||'{}').beneficiariosEstimados||0),beneficiariosB=Number($('proyectoBeneficiariosTerrenoB').value||0);$('proyectoBeneficiarios').value=beneficiariosA+beneficiariosB;actualizarModoJerarquiaProyecto();$('modalDibujoArea').close();origenMapaCrearProyecto=false;mostrarPasoAsistenteProyecto(1);return true;
  }
  ...
  function iniciar() {
    ...
    document.querySelector('.navegacion-etapas-proyecto').addEventListener('click',evento=>{
      ...
      if(paso===3){actualizarIndicadorAsistente(3);renderCronogramaProyecto();mostrarVistaCronogramaProyecto();return;}
    ...
    $('subirAreaIntervencion').addEventListener('click',()=>$('archivoAreaIntervencion').click());
    $('archivoAreaIntervencion').addEventListener('change',evento=>{$('estadoArchivoAreaIntervencion').textContent=evento.target.files[0]?`${evento.target.files[0].name} adjunto.`:'Sin archivo adjunto.';});
    const abrirTerrenoInfluencia=async terreno=>{terrenoActivoProyecto=terreno;origenMapaCrearProyecto=true;vistaCrearProyecto.hidden=true;const selector=$('proyectoZonaObjetivo');selector.replaceChildren(new Option(`${$('proyectoCodigo').value||'BORRADOR'} · ${$('proyectoNombre').value||'Proyecto en creación'}`,'borrador'));$('nombreZonaProyecto').value=`Terreno ${terreno}`;puntosPoligonoUbicacion=[];$('modalDibujoArea').showModal();iniciarMapaUbicacionProyecto();renderPoligonoUbicacion();try{await mostrarDepartamentosMapaProyecto();}catch(error){$('estadoZonaProyecto').textContent='No se pudieron cargar los límites territoriales del Perú.';}};
    $('abrirMapaUbicacionProyecto').addEventListener('click',()=>abrirTerrenoInfluencia('A'));
    $('abrirMapaTerrenoB').addEventListener('click',()=>abrirTerrenoInfluencia('B'));
